...
htsmsg_t *CHTSPConnection::SendAndWait0 ( const char *method, htsmsg_t *msg, int iResponseTimeout )
{
  if (iResponseTimeout == -1)
    iResponseTimeout = Settings::GetInstance2().GetResponseTimeout();
...
{
  std::string user = Settings::GetInstance2().GetUsername();
  std::string pass = Settings::GetInstance2().GetPassword();
