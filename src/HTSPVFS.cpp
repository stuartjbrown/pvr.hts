...
void CHTSPVFS::Connected ( void )
{
  ...
  if (m_fileId != 0)
  {
    Logger::Log2(LogLevel::LEVEL_DEBUG, "vfs re-open file");
    if (!SendFileOpen(true) || !SendFileSeek(m_offset, SEEK_SET, true))
    {
      Logger::Log2(LogLevel::LEVEL_ERROR, "vfs failed to re-open file");
      Close();
    }
  }
}
...
bool CHTSPVFS::Open ( const PVR_RECORDING &rec )
{
  ...
  if (!SendFileOpen())
  {
    Logger::Log2(LogLevel::LEVEL_ERROR, "vfs failed to open file");
    return false;
  }
...
long long CHTSPVFS::Size ( void )
{
  ...
  Logger::Log2(LogLevel::LEVEL_TRACE, "vfs stat id=%d", m_fileId);
  ...
  if (htsmsg_get_s64(m, "size", &ret))
    ...
    Logger::Log2(LogLevel::LEVEL_TRACE, "vfs stat size=%lld", (long long)ret);
...
bool CHTSPVFS::SendFileOpen ( bool force )
{
  ...
  Logger::Log2(LogLevel::LEVEL_DEBUG, "vfs open file=%s", m_path.c_str());
  ...
  {
    Logger::Log2(LogLevel::LEVEL_ERROR, "malformed fileOpen response: 'id' missing");
    m_fileId = 0;
  }
  else
    Logger::Log2(LogLevel::LEVEL_TRACE, "vfs opened id=%d", m_fileId);
...
void CHTSPVFS::SendFileClose ( void )
{
  ...
  Logger::Log2(LogLevel::LEVEL_DEBUG, "vfs close id=%d", m_fileId);
...
long long CHTSPVFS::SendFileSeek ( int64_t pos, int whence, bool force )
{
  ...
  Logger::Log2(LogLevel::LEVEL_TRACE, "vfs seek id=%d whence=%d pos=%lld",
  ...
  if (m == NULL)
  {
    Logger::Log2(LogLevel::LEVEL_ERROR, "vfs fileSeek failed");
    return -1;
  }
  ...
  {
    ret = -1;
    Logger::Log2(LogLevel::LEVEL_ERROR, "vfs fileSeek response: 'offset' missing'");

  /* Update */
  }
  else
  {
    Logger::Log2(LogLevel::LEVEL_TRACE, "vfs seek offset=%lld", (long long)ret);
    m_offset = ret;
  }
...
ssize_t CHTSPVFS::SendFileRead(unsigned char *buf, unsigned int len)
{
  ...
  Logger::Log2(LogLevel::LEVEL_TRACE, "vfs read id=%d size=%d",
    m_fileId, len);
  ...
  if (m == NULL)
  {
    Logger::Log2(LogLevel::LEVEL_ERROR, "vfs fileRead failed");
    return -1;
  }
  ...
  {
    Logger::Log2(LogLevel::LEVEL_ERROR, "malformed fileRead response: 'data' missing");
    read = -1;
