...
void Subscription::SendSubscribe(uint32_t channelId, uint32_t weight, bool restart)
{
  ...
  Logger::Log2(LogLevel::LEVEL_DEBUG, "demux subscribe to %d",    GetChannelId());
  ...
  Logger::Log2(LogLevel::LEVEL_DEBUG, "demux successfully subscribed to channel id %d, subscription id %d", GetChannelId(), GetId());
}
...
void Subscription::SendUnsubscribe(void)
{
  ...
  htsmsg_t *m = htsmsg_create_map();
  htsmsg_add_u32(m, "subscriptionId",   GetId());
  Logger::Log2(LogLevel::LEVEL_DEBUG, "demux unsubscribe from %d", GetChannelId());
  ...
  htsmsg_destroy(m);
  Logger::Log2(LogLevel::LEVEL_DEBUG, "demux successfully unsubscribed from channel id %d, subscription id %d", GetChannelId(), GetId());
}
...
bool Subscription::SendSeek(double time)
{
  ...
  Logger::Log2(LogLevel::LEVEL_DEBUG, "demux send seek %d",      time);
...
void Subscription::SendSpeed(int32_t speed, bool restart)
{
  ...
  Logger::Log2(LogLevel::LEVEL_DEBUG, "demux send speed %d",     GetSpeed() / 10);
...
void Subscription::SendWeight(uint32_t weight)
{
  ...
  Logger::Log2(LogLevel::LEVEL_DEBUG, "demux send weight %u",    GetWeight());
